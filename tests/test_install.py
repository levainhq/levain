...
def test_apply_init_generates_claude_seed_imports_byte_identical(tmp_path: Path, monkeypatch):
    ...
    # The on-demand half is pinned by FORMAT here and sourced from the DATA
    # (ON_DEMAND_SUMMARY), not regenerated by `_on_demand_block` — regenerating it
    # with the function under test would assert the function equals itself.
    expected_on_demand = "\n".join(
        [
            ...
            "",
            *(
                f"- `seed/{name}` — {ON_DEMAND_SUMMARY[name]}"
                for name in ON_DEMAND_SEED
            ),
        ]
    )
    expected = template.replace("{{SEED_IMPORTS}}", expected_block).replace(
        "{{SEED_ON_DEMAND}}", expected_on_demand
    )
    got = (install / "CLAUDE.md").read_text(encoding="utf-8")
    assert got == expected
    assert "@seed/continuity.md" not in got
    assert "@seed/README.md" not in got
    # An on-demand seed is NOT eagerly imported, but IS pointed at. Asserting only
...
def test_apply_init_generates_codex_seed_imports_byte_identical(tmp_path: Path, monkeypatch):
    ...
    assert got == expected
    # Off the numbered read-list, but still pointed at (see the claude twin).
    assert "5. `seed/spore_instructions.md`" not in got
    assert "`seed/spore_instructions.md`" in got
...
def test_apply_init_pack_seed_file_loads_in_both_adapters(tmp_path: Path, monkeypatch):
    ...
    # appended AFTER the base curriculum (the last EAGER base import is memory.md;
    # spore_instructions.md is on-demand and off this list entirely)
    assert claude.index("@seed/memory.md") < claude.index("@seed/audit_method.md")

    cx = tmp_path / "cx"
    cx.mkdir()
    _apply(cx, "codex")
    agents = (cx / "AGENTS.md").read_text(encoding="utf-8")
    assert "5. `seed/audit_method.md` — Pressable Audit Method" in agents
...
def test_copy_activation_patched_hook_still_caught_under_substitution(tmp_path: Path):
    ...
    assert not [m for m in said if "Operator-edited posture.md" in m]
...
def test_copy_activation_unreadable_dir_is_KEPT_in_the_backup(tmp_path: Path):
    ...
    to SCAN every file to decide what to copy, and a file it could not enumerate would be
    destroyed by the swap (it refused instead, which blocked the reinstall). Since
    spore-861 nothing is scanned before the swap: the whole previous tree is renamed into
    the backup, unreadable directory and all, and the reinstall proceeds.
...
def test_a_failed_best_effort_copy_is_ANNOUNCED_not_swallowed(tmp_path: Path):
    ...
    kept = list(install.glob(".levain-activation-prev-*/hooks/_levain_hook.py"))
    assert len(kept) == 1 and "my-wrapper" in kept[0].read_text(encoding="utf-8")
...
def test_backup_rotation_keeps_the_newest_three_trees_and_never_touches_legacy_dirs(
    tmp_path: Path,
):
    """spore-861, ruled 2026-09-13: keep the last three whole-tree backups. Directories
    older releases wrote (a bare timestamp name) may hold the only copy of an edit, so
    the rotation must never select them.

    MUTATION CONTROL: drop the `tree-` prefix filter and the legacy dir is deleted; drop
    the rotation and five trees remain."""
    base = _mk_layer(tmp_path / "base", {"posture.md": "P\n"})
    install = tmp_path / "install"
    dst = install / "activation"
    legacy = install / ".levain" / "backups" / "activation" / "1700000000000000000"
    _mk_layer(legacy, {"posture.md": "AN EDIT ONLY THIS DIR HOLDS\n"})
    _copy_activation_tree([base], dst, base_activation=base)      # fresh: nothing to keep
    for n in range(5):
        (dst / "posture.md").write_text(f"EDIT {n}\n", encoding="utf-8")
        _copy_activation_tree([base], dst, base_activation=base)

    root = install / ".levain" / "backups" / "activation"
    trees = sorted(p for p in root.iterdir() if p.name.startswith("tree-"))
    assert [(t / "posture.md").read_text(encoding="utf-8") for t in trees] == [
        "EDIT 2\n", "EDIT 3\n", "EDIT 4\n"]
...
# ---------- F1: the interview names the BEHAVIOR file (2026-08-01) ----------
#
# The interview fills IDENTITY (world.md + origin.md) and never touches the file
# that DIRECTS the entity. Alex De Groodt, the first outside installer: "I'm not
# sure if it's applying all that I told it." He was right, and nothing in the flow
# said those were different files.


def test_behavior_note_names_posture_as_the_file_that_directs():
    install = Path("/tmp/ent")
    lines = _behavior_note_lines(install, has_posture=True)
    text = "\n".join(lines)
    assert "activation/posture.md" in text
    # It must state BOTH halves. Naming posture.md while implying the interview
    # filled it would leave the original wrong belief intact.
    #
    # Asserts the seed DIRECTORY, not "world.md and origin.md": a pack can change
    # which files the interview renders (packs.py `render`), so naming the two base
    # targets is false for a packed install (codex, L3). The posture.md PATH is
    # stable even under an activation-tree override, so it stays named exactly.
    assert "seed/" in text
    assert "CONTEXT" in text
    assert "did NOT" in text
    # The distinction that resolves Alex's confusion, in words: known vs enforced.
    assert "ENFORCED" in text and "known" in text


def test_behavior_note_does_not_name_a_file_a_hookless_install_lacks():
    """openhands installs NO activation tree, so pointing at activation/posture.md
    would trade one wrong belief for another — a path that is not on disk."""
    lines = _behavior_note_lines(Path("/tmp/ent"), has_posture=False)
    text = "\n".join(lines)
    assert "activation/posture.md" not in text
    assert "seed/partnership.md" in text
    assert "did NOT" in text


def test_next_steps_carries_the_behavior_note_for_both_surfaces(tmp_path: Path):
    """The note lives in _next_steps_lines, which the terminal AND the web init
    form both render — one site, so the two surfaces cannot drift apart."""
    install = tmp_path / "ent"
    (install / "activation").mkdir(parents=True)
    (install / "activation" / "posture.md").write_text("## p\n", encoding="utf-8")
    text = "\n".join(_next_steps_lines(install, "claude-code"))
    assert "What the interview did NOT set:" in text
    assert "activation/posture.md" in text
    # BEFORE the action list: a correction placed after "Next steps" goes unread.
    assert text.index("What the interview did NOT set:") < text.index("Next steps:")


def test_next_steps_behavior_note_follows_the_install_on_disk(tmp_path: Path):
    """The branch is chosen from the FILESYSTEM, not from the adapter name — an
    install whose activation tree failed to land must not be told to edit it."""
    install = tmp_path / "ent"
    install.mkdir()
    text = "\n".join(_next_steps_lines(install, "claude-code"))
    assert "activation/posture.md" not in text
...
def test_fill_seed_on_demand_requires_exactly_one_placeholder():
    """Same honesty floor as {{SEED_IMPORTS}}: a template missing the placeholder
    writes a carrier whose on-demand files are unreachable, and a doubled one
    duplicates the block."""
    with pytest.raises(InitError, match="SEED_ON_DEMAND"):
        _fill_seed_on_demand("no placeholder here\n", "- `seed/x.md` — y")
    with pytest.raises(InitError, match="exactly one"):
        _fill_seed_on_demand("a {{SEED_ON_DEMAND}} b {{SEED_ON_DEMAND}} c", "X")
    assert _fill_seed_on_demand("a\n{{SEED_ON_DEMAND}}\nb", "BLOCK") == "a\nBLOCK\nb"


def test_fill_seed_on_demand_drops_the_block_when_nothing_is_on_demand():
    """An EMPTY on-demand set is legitimate (nothing moved behind a load), unlike
    an empty import block — so the placeholder and its blank line disappear rather
    than leaving a hole in the rendered carrier."""
    assert _fill_seed_on_demand("a\n{{SEED_ON_DEMAND}}\nb", "") == "a\nb"
    # end-of-file placeholder (no trailing newline) is covered too
    assert _fill_seed_on_demand("a\n{{SEED_ON_DEMAND}}", "") == "a\n"


def test_on_demand_block_carries_the_retention_summary_not_a_bare_filename():
    """A bare pointer would silently kill the layer it points at: the spore TOOLS
    stay in context, but the discipline that makes them fire lives only in the
    file. The block must carry the retained content."""
    root = Path(__file__).resolve().parent.parent / "levain" / "templates" / "seed"
    entries = [
        SeedEntry(name=n, path=root / n, mode="verbatim") for n in ON_DEMAND_SEED
    ]
    block = _on_demand_block(entries)
    for name in ON_DEMAND_SEED:
        assert f"`seed/{name}`" in block
        assert ON_DEMAND_SUMMARY[name] in block
    # the operative instruction, not just a description of the file
    assert "spore_add" in block and "must close" in block
    assert _on_demand_block([]) == ""


# ---------- L3 findings (2026-08-01, codex + glm) ----------


def test_pack_override_of_an_on_demand_file_does_not_inherit_the_base_summary(tmp_path: Path):
    """codex + glm L3, CONVERGED: `on_demand_entries` resolves to the WINNING layer,
    but ON_DEMAND_SUMMARY is keyed by FILENAME — so a pack overriding the file's
    content would keep the pointer asserting the BASE discipline over pack-authored
    text. Confidently describing a procedure the file does not contain is worse than
    describing none, so a non-base winner falls back to the file's own H1."""
    pack_file = tmp_path / "spore_instructions.md"
    pack_file.write_text("# Domain Loop Protocol\n\nentirely different.\n", encoding="utf-8")
    entry = SeedEntry(name="spore_instructions.md", path=pack_file, mode="verbatim")

    block = _on_demand_block([entry])
    assert "Domain Loop Protocol" in block
    # the base discipline is NOT asserted over the pack's file
    assert "spore_add" not in block
    assert ON_DEMAND_SUMMARY["spore_instructions.md"] not in block


def test_base_seed_still_gets_its_full_retention_summary():
    """The control for the test above: the BASE file must keep the real summary, or
    the pack fix would have silently degraded every ordinary install."""
    root = Path(__file__).resolve().parent.parent / "levain" / "templates" / "seed"
    entry = SeedEntry(
        name="spore_instructions.md", path=root / "spore_instructions.md", mode="verbatim"
    )
    block = _on_demand_block([entry])
    assert ON_DEMAND_SUMMARY["spore_instructions.md"] in block
    assert "spore_add" in block


def test_fill_seed_on_demand_empty_does_not_double_the_blank_line():
    """glm L3: the shipped template spaces the placeholder with a blank line, a shape
    the original unit test did not cover. Dropping the block must not leave two."""
    template = "head\n\n{{SEED_ON_DEMAND}}\n"
    assert _fill_seed_on_demand(template, "") == "head\n"


def test_fill_seed_on_demand_empty_leaves_an_inline_placeholder_line_intact():
    """codex L3: the empty-drop is scoped to a placeholder that OWNS its line. An
