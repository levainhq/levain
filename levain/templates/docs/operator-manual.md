...
- **`levain doctor`** — the health check. Run it after install, after an upgrade, or any time something feels off. It checks that the machinery is wired correctly *and* that your identity file actually got filled. Two different things can go wrong there and it sees both: a field never filled in, and a field filled with the *wrong* answer because the answers landed one slot off. If your name or your partner's is missing it fails outright; anything else you chose to leave blank it reports rather than condemns, because a thinner seed is still a working partnership. A green doctor means the plumbing works and the seed is intact; it does not mean the memory is *correct* (that's §7, and it's yours).
...
- **`levain update`** — the one move. It brings the memory library to the version your Levain was tested against, and surfaces — for your review, never a silent rewrite — any changes to the standing instructions your partner runs on. It does not rewrite your partner's activation files; the command that replaces those is `levain init --force`, which backs up your edits to `posture.md` and `recency_directives.md` first. Run it when you upgrade Levain, or when `doctor` says the versions have drifted apart.
